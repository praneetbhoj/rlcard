...
def plot_curve(csv_path, save_path, algorithm):
    ...
    with open(csv_path) as csvfile:
        ...
        x = np.linspace(min(episodes), max(episodes), max(episodes) - min(episodes))
        ax1.plot(episodes, win_rates, label=algorithm)
        a, b = np.polyfit(np.log(episodes), win_rates, 1)
        ax1.plot(x, a * np.log(x) + b, 'r--')
        ax1.set(xlabel='episode', ylabel='win rate')
        ax1.legend()
        ax1.grid()
        ax2.plot(episodes, expected_earnings, label=algorithm)
        a, b = np.polyfit(np.log(episodes), expected_earnings, 1)
        ax2.plot(x, a * np.log(x) + b, 'r--')
